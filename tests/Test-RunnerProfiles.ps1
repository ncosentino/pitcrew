...
foreach ($event in $journalV12.events) {
    $event |
        Add-Member -NotePropertyName firstObservedAt `
            -NotePropertyValue $event.observedAt
    $event |
        Add-Member -NotePropertyName lastObservedAt `
            -NotePropertyValue $event.observedAt
    $event |
        Add-Member -NotePropertyName occurrenceCount -NotePropertyValue 1
}
...
foreach ($event in $legacyJournalV19.operationJournal.events) {
    $event.PSObject.Properties.Remove('firstObservedAt')
    $event.PSObject.Properties.Remove('lastObservedAt')
    $event.PSObject.Properties.Remove('occurrenceCount')
}
Add-Check (
    ($legacyJournalV19 | ConvertTo-Json -Depth 20) |
        Test-Json -SchemaFile $observedStateSchemaPath
) 'Manager contract nineteen rejected legacy unaggregated journal events.'

$missingJournalAggregateV20 = (
    $legacyJournalV19 |
        ConvertTo-Json -Depth 20 |
        ConvertFrom-Json -Depth 20
)
$missingJournalAggregateV20.managerContractVersion = 20
Add-Check (-not (
    ($missingJournalAggregateV20 | ConvertTo-Json -Depth 20) |
        Test-Json `
            -SchemaFile $observedStateSchemaPath `
            -ErrorAction SilentlyContinue
)) 'Manager contract twenty accepted a journal event without aggregate fields.'

$completeJournalAggregateV20 = (
    New-RunnerHostAdmissionSchemaFixture `
        -ManagerContractVersion 20 `
        -HostAdmission $availableHostAdmissionV19
)
Add-Check (
    ($completeJournalAggregateV20 | ConvertTo-Json -Depth 20) |
        Test-Json -SchemaFile $observedStateSchemaPath
...
$invalidOccurrenceCountV20.operationJournal.events[0].occurrenceCount = 0
Add-Check (-not (
    ($invalidOccurrenceCountV20 | ConvertTo-Json -Depth 20) |
        Test-Json `
            -SchemaFile $observedStateSchemaPath `
            -ErrorAction SilentlyContinue
)) 'Manager contract twenty accepted a zero journal occurrence count.'
