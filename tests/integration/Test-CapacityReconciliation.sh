...
start_legacy_compose() {
    (
        ...
        PITCREW_MANAGER_CONTRACT_VERSION="20" \
            docker compose \
...
[ "$(jq -r '.managerContractVersion' "${LEGACY_OBSERVED_STATE}")" -eq 20 ] || {
    echo "Direct Compose observed state did not report manager contract version twenty." >&2
    exit 1
}
...
[ "$(jq -r '.managerContractVersion' "${OBSERVED_STATE}")" -eq 20 ] || {
    echo "Observed state did not report manager contract version twenty." >&2
    exit 1
}
